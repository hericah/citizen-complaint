    ...
    <StaffLayout :user="user">
      <div class="mb-8">
        <div class="flex items-start justify-between">
          <div>
            <h2 class="text-2xl md:text-3xl font-bold text-gray-800">Tambah Unit OPD</h2>
            <p class="text-sm text-gray-500 mt-1">Buat unit baru untuk OPD</p>
          </div>
      ...
      <div class="bg-white rounded-xl shadow-lg p-8 max-w-2xl">
        <form @submit.prevent="submit" class="space-y-6">
          ...
          <button type="submit" :disabled="form.processing" class="w-full px-4 py-3 bg-blue-500 hover:bg-blue-600 text-white font-medium rounded-lg transition disabled:opacity-50">{{ form.processing ? 'Menyimpan...' : 'Simpan Unit' }}</button>
        </form>
      </div>
    </StaffLayout>
  </div>
...
defineProps({ user: Object, opdList: Array });

const form = useForm({ opd_id: "", nama_unit: "", kode_unit: "" });

const submit = () => {
  form.post('/master/unit-opd', {
    onSuccess: () => {
      Swal.fire({ icon: 'success', title: 'Unit Berhasil Ditambahkan', confirmButtonColor: '#3B82F6' }).then(() => {
